...
namespace BlobStorageApp
{
    class Program
    {
        ...
        private static readonly string _connectionString = "DefaultEndpointsProtocol=https;AccountName=demoazuredotnetsdk;AccountKey=bqnX5dj1c/CFo2h6YyOlufaccmvMpD0olLAR4znQHLEAD2axA3g6bVlINb4BfRsSCLeSv1WkuLh3+AStb337og==;EndpointSuffix=core.windows.net";
        ...
        private async static Task Main(string[] args)
        {
            ...
            catch (RequestFailedException exception)
            {
                ...
                if (exception.ErrorCode == "ContainerBeingDeleted")
                {
                    ...
                    Console.WriteLine("> This takes usually up to 45 seconds.");
        ...
        private static async Task CreateContainerAndUploadBlobAsync()
        {
            ...
            Console.WriteLine($"2. Uploading the image file '{blobClient.Name}'");
            Console.WriteLine($"   > {blobClient.Uri}");

            using FileStream fileStream = File.OpenRead("filetoupload.html");

            object value =  blobClient.Upload(fileStream);
        }
